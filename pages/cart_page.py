...
class CartPage(BasePage):
    def get_item_names(self):
        item_names = self.driver.find_elements(*CartLocators.ITEM_NAME)
        return [item.text for item in item_names]

    def get_item_prices(self):
        item_prices = self.driver.find_elements(*CartLocators.ITEM_PRICE)
        return [item.text for item in item_prices]

    def get_quantity(self):
        quantity_element = self.driver.find_element(*CartLocators.ITEM_QUANTITY)
        return quantity_element.text
